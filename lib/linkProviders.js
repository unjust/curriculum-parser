...
export const parseLinks = (node) => {
  // if the node type is a link and said the link has a title
  // with the last word being different than the string '/no-iframe/'
  // we'll transform said link
  if (node.type === 'link' && node.title?.split(' ').pop() !== '/no-iframe/') {
